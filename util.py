...
def question_embeddings(questions, vocabs, Xml):
    ...
    # Load word2vec
    print("preparing the knowledge database")
    ...
    for index in range(len(questions)):
        ...
        row = None
        if Xml:
            row = questions[index].childNodes[0].data
        else:
            row = questions[index]
        for word in text_to_word_list(row):
            # Check for unwanted words
            if word in stops:
                continue
...
def zero_padding(data, max_seq_length):
    return pad_sequences(data, padding='pre', truncating='post', maxlen=max_seq_length)
